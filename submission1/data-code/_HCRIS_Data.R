...
## create v2010 data
source('submission1/data-code/HCRISv2010.R')

## compute uncomp_care and collapse depreciation columns into accum_dep,
## parse dates, and fix sign conventions; then dedup exact-match fiscal years
## by taking the most recently created report
final.hcris <- final.hcris.v2010 %>%
  ...
  arrange(desc(date_created), .by_group = TRUE) %>%
  slice(1) %>%
  ungroup() %>%
